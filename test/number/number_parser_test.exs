defmodule Cldr.Number.Parsing.Test do
  ...
  test "scanning strings that have symbols in them" do
    assert Cldr.Number.Parser.scan("a string, which I think. Well, sometimes not £1_000_000.34") ==
             ["a string, which I think. Well, sometimes not £", 1_000_000.34]
  end

  test "parse a decimal" do
    {:ok, parsed} =
      Cldr.Number.Parser.parse("1.000,00",
        number: :decimal,
  ...
  test "Parse a list of numbers separated by comma" do
    assert Cldr.Number.Parser.scan("Here's my number list: 1111, 2222, 3333, 4444, 55,55",
             number: :decimal
           ) ==
             [
               "Here's my number list: ",
               Decimal.new(1111),
               ", ",
               Decimal.new(2222),
               ", ",
  ...
  test "Resolving currencies in a string" do
    scanned =
      Cldr.Number.Parser.scan(
        "Lets try this 123 US dollars, a bunch of US dollars 23 and 345 euros"
      )
    ...
    scanned =
      Cldr.Number.Parser.scan(
        "Lets try this 123 US dollars, a bunch of swiss francs 23 and 345 euros"
      )
